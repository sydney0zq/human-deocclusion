...
class MaskWeightedCrossEntropyLoss(nn.Module):
    ...
    def forward(self, predict, target, mask):
        ...
        #mask = mask.byte()         # lead to torch.uint8 index warning
        mask = mask.bool()
        ...
        loss_inmask = nn.functional.cross_entropy(
            predict_inmask, target_inmask, reduction='sum')
        loss_outmask = nn.functional.cross_entropy(
            predict_outmask, target_outmask, reduction='sum')
...
class InpaintingLoss(nn.Module):
    ...
    def forward(self, input, mask, output, gt):
        ...
        if output.shape[1] == 3:
            ...
            raise ValueError('only gray or rgb')
        ...
        # Total variation loss, 用來確保周遭pixel是否平滑化
